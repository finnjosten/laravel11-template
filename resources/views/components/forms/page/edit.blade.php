<form method="POST" class="vlx-form" action="{{ route("dashboard.pages.update", $page->id) }}" id="pageForm">
    ...
    <div class="vlx-form__box vlx-form__box--hor">
        ...
        <x-forms.input label="Slug" id="page-slug" type="text" name="slug" value="{{ old('slug', $page->slug ?? '') }}" desc="Use {none} to dictate that a page should be accessable directly on / (like a homepage). The page cant be reached via /{none}" />
    </div>
